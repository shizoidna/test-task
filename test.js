...
function startRecursive(users, lastUnresolvedIndex, total) {
  if (lastUnresolvedIndex >= users.length) {
    return Promise.resolve(total);
  }

  const batchSize = 10;
  const nextLastUnresolvedIndex = lastUnresolvedIndex + batchSize;
  const requests = users
    .slice(lastUnresolvedIndex, nextLastUnresolvedIndex)
    .map(({ id }) => processUser(id));

  return Promise.all(requests)
    .then((response) => startRecursive(
      users,
      nextLastUnresolvedIndex,
      response.reduce((acc, id) => acc + id, total)
    ));
}

startRecursive(users, 0, 0)
  .then(total => {
